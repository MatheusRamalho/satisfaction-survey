...
export function Modal({
    ...
    return (
        <div
            className={`z-20 fixed inset-0 size-screen overflow-hidden items-center justify-center bg-zinc-950/80 backdrop-blur-sm ${
                isItToShowModal ? 'flex' : 'hidden'
            }`}
        >
            <div className="h-auto w-11/12 md:w-3/4 lg:w-1/2 overflow-auto rounded-lg bg-zinc-900 border border-zinc-800 shadow-xl">
                <div className="relative w-full h-20 bg-green-700 flex items-center justify-center border-b border-b-green-700">
                    <button
                        className="absolute top-1/2 right-4 -translate-y-1/2 cursor-pointer size-7 border-none rounded bg-green-500 hover:shadow-xl text-white text-xl hover:bg-green-700"
                ...
                <div className="h-[calc(100%-56px)] p-8 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-lg text-zinc-300 leading-10">
                        A nota que você atribuiu foi
                        <strong> {note}</strong>
                    </p>

                    {note <= 4 && (
                        <p className="text-base text-zinc-500 leading-10">
